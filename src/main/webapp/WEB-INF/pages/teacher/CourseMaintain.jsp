...
<%
  ...
  String url=status==null?request.getContextPath()+"/CourseMaintain?":request.getContextPath()+"/CourseMaintain?status="+status+"&&";
    String redirectUrl=status==null?"/CourseMaintain?":"/CourseMaintain?status="+status+"&&";

    if(!"3".equals(status)&&!"1".equals(status)&&!"2".equals(status)&&!"4".equals(status)&&!"5".equals(status))
        status="0";
%>
<!DOCTYPE html>
<html lang="en">
...
--%>
                      <td><a class="course-preview" href="javascript:var fileType='${slide.fileType}';if(fileType=='pdf'||fileType=='video'||fileType=='image')location.href='<%=request.getContextPath()%>/Preview/${slide.sId}';else location.href='<%=request.getContextPath()%>/${slide.filePath}'">${slide.fileName}</a></td>
                      ...
                      <td>
                          <c:choose>
                             <%-- <c:when test="${slide.state==1}">
                                  ...
                                  <a class="operation check-operation" data-toggle="modal" data-target="#fail${slide.sId}"><label><span class="glyphicon glyphicon-remove"></span> 驳回</label></a>
                                  <div class="modal fade" id="fail${slide.sId}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                      <div class="modal-dialog">
                                          <div class="modal-content">
                                              ...
                                              <div class="modal-body text-center">
                                                  <textarea class="form-control" id="opinion${slide.sId}" placeholder="请输入驳回意见">${slide.opinion}</textarea>
                                              </div>
                                              <div class="modal-footer">
                                                  <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                              ...
                              <c:when test="${slide.state==1}">
                                  ...
                                  <a class="operation check-operation" data-toggle="modal" data-target="#update${slide.sId}"><label><span class="glyphicon glyphicon-remove"></span>更正文件</label></a>
...
<script>
  ...
  function filterStatus(status){
    location.href="<%=request.getContextPath()%>/CourseMaintain?status="+status;
  }
